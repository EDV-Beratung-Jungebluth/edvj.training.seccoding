...
namespace NormalizeStrings
{
    class Program
    {
        public static void Main()
        {
            ...
            string s1 = "\u0063\u0301\u0327\u00BE";
            string sNfc = s1.Normalize(NormalizationForm.FormC);
            ...
            Show("s2", s2);
            Console.WriteLine();
            Console.WriteLine($"s1 roh:        {s1}");
            Console.WriteLine($"s1 NFC:        {sNfc}");
            Console.WriteLine($"Codepoints s1: {string.Join(" ", s1.EnumerateRunes().Select(r => r.ToString() + " (U+" + r.Value.ToString("X4") + ")"))}");
            Console.WriteLine($"Codepoints NFC:{string.Join(" ", sNfc.EnumerateRunes().Select(r => r.ToString() + " (U+" + r.Value.ToString("X4") + ")"))}");
            Console.ReadLine();
        }
